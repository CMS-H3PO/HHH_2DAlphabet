...
if __name__ == '__main__':
    ...
    bestOrder = {"{}_semiboosted_VR".format(options.year):"2" if options.year=="2017" else "1"}
    for working_area in ["{}_semiboosted_VR".format(options.year)]:
        ...
        for polyOrder in ["0","1","2","3"]:
            if options.year == "2017":
                if polyOrder in ["1","2"]:
                    test_fit(working_area,polyOrder,defMinStrat=2, rMin=-5, rMax=5)
                elif polyOrder in ["3"]:
                    test_fit(working_area,polyOrder,defMinStrat=1, rMin=-5, rMax=10)
                else:
