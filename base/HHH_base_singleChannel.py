...
def _load_fit_rpf(working_area,polyOrder,json_file):
    twoD_blindFit = TwoDAlphabet(working_area,json_file, loadPrevious=True)
    params_to_set = twoD_blindFit.GetParamsOnMatch('rpf.*', f'{polyOrder}_area', 'b')
    return {k:v['val'] for k,v in params_to_set.items()}
...
def test_fit(working_area,polyOrder,sigName=None,defMinStrat=0,rInit=0,rMin=-1,rMax=10,setParams={},extra='',add_tt_mistag_sf=True):
...
def test_limit(working_area,polyOrder,json_file,blind=True,defMinStrat=0,extra=''):
    ...
    params_to_set = _load_fit_rpf(working_area,polyOrder,json_file)
    print(params_to_set)
...
def test_plot(working_area,polyOrder):
    ...
    twoD = TwoDAlphabet(working_area, '%s/runConfig.json'%working_area, loadPrevious=True)
    subset = twoD.ledger.select(_select_bkg, polyOrder)
    twoD.StdPlots(f'{polyOrder}_area', subset)
...
def test_GoF(working_area,polyOrder):
    ...
    twoD.GoodnessOfFit(
        f'{polyOrder}_area', ntoys=5000, freezeSignal=False,
        condor=True, njobs=100, lorienTag=True
    )
